...
if special==0:
    ...
    if find_dist(group1)>find_dist(group2):
        base=average_points(group1)
        middle=average_points([average_points(group1),average_points(group2)])
        cv2.circle(grad,base,5,255,3)
        cv2.circle(grad,middle,5,255,3)
        cv2.line(grad,middle,base, 255,1)
    else:
        base=average_points(group2)
        middle=average_points([average_points(group1),average_points(group2)])
        cv2.circle(grad,base,5,255,3)
        cv2.circle(grad,middle,5,255,3)
        cv2.line(grad,middle,base, 255,1)
